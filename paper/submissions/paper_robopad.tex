...
\title{Curation Metrics Are a Post-Training Decision:\\
Auditing Trajectory Smoothness for Adapting Robot Foundation Models}
...
duration-matched band, while jerk rises from 0.431 to 0.896. For
post-training pipelines that filter, reweight, or select demonstrations
before fine-tuning a pretrained policy, the practical consequence is direct:
the curation step can silently discard the corrective, high-effort
demonstrations that fine-tuning most needs, while keeping the smooth,
low-information ones. Our results extend the detection-vs-curation
decoupling to named, published metrics, on operator labels rather than
injected defects, and to a benchmark where the length confound is excluded
by construction rather than by truncation.
\end{abstract}

\section{Introduction}

A large share of post-training work for robot foundation models runs on
curated demonstration subsets: filtering, reweighting, or ranking
trajectories before fine-tuning, whether the source is human teleoperation,
autonomous rollouts, or a mix of both. The curation step is rarely evaluated
as carefully as the fine-tuning step that follows it, and the metrics used
to perform it are typically validated once, on clean data, and then reused
across deployments without re-checking whether they still measure what they
claim to measure. This paper is a case study in what goes wrong when that
assumption fails: a published, named curation metric that looks reliable on
successful demonstrations, and is actively harmful once the pool it filters
contains the failures and near-misses that post-training adaptation
pipelines actually have to work with.
...
\paragraph{Reproducibility.} All datasets are the public robomimic
release~\citep{robomimic}; SAL follows its published formula and TED is
reimplemented from the \rinse{} appendix as described in
Section~\ref{sec:setup}. Metric implementations, scoring scripts, and
training configurations will be released at an anonymized repository upon
acceptance and de-anonymized in the camera-ready or public posting.

\paragraph{Use of AI assistance.} An AI assistant was used for code
scaffolding, LaTeX drafting, and editing support during this project. All
experiments were designed, executed, and verified by the author(s), and all
reported numbers were produced by the released code and checked against the
raw outputs.
