...
\url{https://github.com/imjbassi/smoothness-audit} (released on publication).}}
...
subset, every metric-curated condition underperforms both an uncurated,
size-matched baseline and a random 50\% subset; TED curation nearly halves
downstream success relative to no curation at all (0.216 vs.\ 0.408).
...
smoothness metric measures commitment with the sign flipped. Our results extend the
detection-vs-curation decoupling to named, published metrics, on operator
labels rather than injected defects, and in a benchmark where the length
...
jerk retains signal. Every metric-curated training condition underperforms
both random subsampling and no curation at all, with TED curation nearly
halving downstream success relative to doing nothing. Skill and success are different targets, and the
curation literature validates on one while deploying against the other.
...
framing and extend it to published metrics, real labels, and a
natively fixed-length benchmark. Where they control length by truncating to a
fixed prefix, our can/mg experiments require no such intervention.
...
Can. \emph{can/mh}: 300 successful demonstrations from six human operators
...
intervals (2000 resamples). For skill we report better-vs-worse AUROC; for
...
and inverts to 0.359 in the tightest band. Notably, TED does \emph{not}
...
can/mg fixes every episode at 150 steps, so no metric can exploit duration.
Against the success label (716 of 3900 successful), jerk achieves 0.706, SAL
0.454, path length 0.421, and TED 0.376 (Figure~\ref{fig:main}b). The
length baseline is exactly 0.500, as it must be. Per-timestep normalization
leaves every AUROC unchanged, as expected under a constant divisor, which we
report as a consistency check.

Two readings of this table matter. First, both of \rinse{}'s metrics are at
or below chance on the label practitioners actually deploy against.
TED's inversion means that filtering by TED \emph{preferentially retains
failed demonstrations}. Second, the ordering among metrics reverses relative
to the uncontrolled skill setting: the metric that looked worst there (jerk)
is the only one with signal here.
...
\midrule
oracle (successes only) & 716 & 0.868 $\pm$ 0.023 & [0.852, 0.888] \\
\midrule
no curation             & 1950 & 0.408 $\pm$ 0.077 & [0.348, 0.468] \\
random 50\%             & 1950 & 0.392 $\pm$ 0.056 & [0.348, 0.436] \\
\midrule
SAL                     & 1950 & 0.348 $\pm$ 0.081 & [0.288, 0.408] \\
jerk                    & 1950 & 0.320 $\pm$ 0.032 & [0.296, 0.344] \\
path length             & 1950 & 0.296 $\pm$ 0.030 & [0.272, 0.320] \\
TED                     & 1950 & 0.216 $\pm$ 0.043 & [0.184, 0.252] \\
\bottomrule
\end{tabular}
\caption{Downstream GMM-BC policy success on can/mg after filtering to a
fixed 1950-demonstration pool by each metric, 5 seeds each. Every
metric-curated condition falls below both no-curation and random
subsampling; TED curation nearly halves success relative to no curation.
Oracle has a smaller pool (716, the number of true successes) and is a
ceiling reference, not a matched condition.}
\label{tab:training}
\end{table}

Table~\ref{tab:training} answers the question Section~\ref{sec:detection}
could only pose. Every metric-curated condition underperforms both an
uncurated, size-matched baseline and a random 50\% subset of the same size.
TED curation is the most damaging intervention in the grid: it drops best
rollout success to 0.216 [0.184, 0.252], a confidence interval that does not
overlap random subsampling's [0.348, 0.436], nearly halving downstream
performance relative to training on the full, unfiltered pool. Path length
is similarly and clearly separated from random (0.296 [0.272, 0.320]). SAL
and jerk are directionally below random and no-curation as well, but their
intervals overlap those baselines, so we report this as consistent with, not
statistically distinguishable from, underperforming random selection at
$n{=}5$ seeds.

The downstream ordering mostly, but not perfectly, tracks the detection
ordering from Section~\ref{sec:detection}: TED is worst in both, and path
length is weak in both. Jerk had the best detection AUROC on mg (0.706) but
is narrowly \emph{outperformed} downstream by SAL (0.320 vs.\ 0.348), whose
detection AUROC was near chance (0.454). We do not read this as
contradicting the main claim; both are clearly below random and no
curation, and detection AUROC on a 50\%-keep threshold need not perfectly
order two conditions that are both close to a no-information floor. It
does mean detection accuracy is at best a coarse predictor of curation
value, consistent with the decoupling reported
by~\citep{bedi-policy} on a different testbed. The practical
conclusion is unambiguous regardless of the SAL/jerk ordering: on this
benchmark, filtering training data by any of the four trajectory-statistic
metrics we tested produces a worse policy than not filtering at all.
...
This audit covers one task family (robomimic Can) and one policy class
(GMM behavior cloning); the detection results are policy-free but the
downstream results may differ under sequence models or diffusion policies.
...
role in our argument is corroborative, not primary. The oracle training
...
metric inverts. Training on the metrics' own curated subsets confirms the
practical stakes: every metric we tested produces a policy worse than
training on the full, unfiltered pool, with TED curation nearly halving
downstream success relative to doing nothing at all.
