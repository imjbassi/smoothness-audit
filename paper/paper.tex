...
smoothness metric measures commitment with the sign flipped. Our results
...
halving downstream success relative to doing nothing. Skill and success are
...
intervals overlap those baselines, so we report this as consistent with, but
not statistically distinguishable from, underperforming random selection at
$n{=}5$ seeds.
...
length is weak in both. Jerk had the best detection AUROC on can/mg (0.706)
but is narrowly \emph{outperformed} downstream by SAL (0.320 vs.\ 0.348),
whose detection AUROC was near chance (0.454). We do not read this as
contradicting the main claim; both are clearly below random and no
curation, and detection AUROC at a 50\%-keep threshold need not perfectly
