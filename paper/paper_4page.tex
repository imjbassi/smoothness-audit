...
below chance against task success, and filtering training data by any of
them yields a \emph{worse} policy than not filtering at all. Whether a
duration control is applied is not a robustness detail: it decides the sign
of the reported result. We argue it belongs in the minimum reporting set for
any demonstration-scoring claim.
\end{abstract}
...
1950 demonstrations, and train GMM behavior-cloning policies (5 seeds each,
50 rollouts per evaluation, best checkpoint per seed). Every metric-curated
condition lands below both an uncurated size-matched control (0.408) and a
random 50\% subset (0.392): SAL 0.348, jerk 0.320, path length 0.296, TED
0.216. TED's interval [0.184, 0.252] does not overlap random subsampling's
[0.348, 0.436] --- curating by TED nearly halves downstream success relative
to doing nothing. An oracle keeping only true successes reaches 0.868,
confirming the signal exists in the pool and the metrics are simply not
finding it. Full grid and CIs in supplementary.
...
rewarding smoothness is measuring commitment with the sign flipped, which is
also why jerk is the one baseline that retains signal.
...
such as LIBERO. One policy class (GMM behavior cloning); detection results
are policy-free, downstream results may differ under diffusion or sequence
models. TED is our reimplementation from the published appendix, not the
authors' code; it agrees broadly with SAL's ranking on clean data as a
sanity check, but a discrepancy cannot be excluded. The tightest duration
bands are small ($n{=}40$, $n{=}30$), and we treat them as the endpoint of a
trend rather than as standalone evidence. Failures in can/mg are
machine-generated; human failure modes may differ, and a human-failure pool
...
\bottomrule
\end{tabular}
\caption{Downstream GMM-BC policy success on can/mg, 5 seeds per condition.
Architecture: MLP $1024{\times}1024$, 5 GMM modes, batch 100, 600 epochs of
500 gradient steps, learning rate $10^{-4}$; 50 rollouts (horizon 400) every
150 epochs, best checkpoint per seed. The oracle pool is smaller (716, the
number of successes that exist) and is a ceiling reference, not a matched
condition.}
\end{table}

SAL and jerk are directionally below random and no-curation but their
intervals overlap those baselines; we report this as consistent with, but
not statistically distinguishable from, underperforming random selection at
$n{=}5$ seeds. TED and path length are separated from random with
non-overlapping intervals. The downstream ordering tracks the detection
ordering imperfectly --- jerk has the best detection AUROC on can/mg (0.706)
but is narrowly outperformed downstream by SAL, whose detection AUROC is
near chance --- which is itself consistent with the detection-vs-curation
