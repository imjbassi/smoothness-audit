...
for multiple comparisons). Whether a duration control is applied is not a
robustness detail: it decides the sign of the reported result. We argue it belongs in the minimum reporting set for
...
models. A pilot diffusion-policy replication (5 conditions, 3 seeds, reduced
epochs/rollouts for tractability) showed substantially lower absolute
success everywhere, including the oracle (0.489 vs.\ 0.892 for BC),
indicating an under-converged configuration rather than an architecture
effect; we do not report its numbers and flag a compute-matched replication
as the clearest follow-up. TED is our reimplementation from the published
