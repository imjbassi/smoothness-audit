...
% over unchanged.
% Anonymized 8-page submission for "Oops, I Erred" workshop (CoRL 2026).
% Reframed toward imperfect-data curation; body content otherwise matches
% the full audit paper. Code/data link withheld for double-blind review.
...
\title{A Curation Metric That Cannot Tell Failure From Success:\\
Auditing Trajectory Smoothness on Imperfect Demonstrations}
...
duration-matched band, while jerk rises from 0.431 to 0.896. The practical
upshot for anyone working with imperfect data is direct: a metric built and
validated on successful trajectories can fail to recognize failure when it
actually appears in the pool, and can do so silently, ranking the failure as
the cleaner demonstration. Our results extend the detection-vs-curation
...
Real demonstration pools are imperfect: they mix successful executions with
failures, near-misses, and out-of-distribution attempts, and a growing
literature proposes automatic scores to separate the useful demonstrations
from the rest. This paper asks a basic question about that literature: does
a scoring metric that is built and validated on clean, successful data
actually recognize failure when a real failure is placed in front of it? We
show, for one widely-motivated family of scores, that the answer is no, and
that the failure to recognize failure is not a minor miscalibration but a
sign flip large enough to make training on the curated subset worse than
training on the uncurated pool.
